...
\item \underline{Map from CWSNs to transactions, $W_i$}: This associates CWSNs with either their corresponding ongoing write or write response that has not been acknowledged. Define $W_i^{\text{key}}|cid$ as the maximal subset of $W_i^{\text{key}}$ such that all of its elements have prefix $cid$. This additional per-client bookkeeping is essential to guaranteeing exactly-once semantics in the presence of server failures and message loss (\S \ref{wotxn}).
...
\begin{enumerate}[itemsep=1pt]
\item \underline{Log sequence number of the most recently completed sub-transaction, $shExec(j)$}: This is analogous to the queues and $exec(\cdot)$ in the transaction manager nodes. 
...
\par First, the client invokes $\text{SessionRuntimeAppend}$ through a wrapper API, which handles timeout and retries. \\
\begin{procedure}[H]
  ...
  $ackBound := \min(InProg_{cid})$ \\ % Could optimize this to clean specific intervals
...
\begin{procedure}[H]
  ...
  \uIf{$cwsn_T \leq \max(W_i^{\text{key}}|cid)$}{
    ...
    \uIf{\upshape $i = 1 \wedge W_i[cwsn_T] = \text{resp}\{\text{STATUS}\}$}{
      $\text{Send}_{\text{cid}}(\text{SessionResp}, \{\text{STATUS}\})$\tcp{handle case when $i$ is head}
    }
    \textbf{return}    
  } \Else {
  ...
  \uIf{$i = n$}{ 
    \ForEach{\upshape$j \in C_i[\text{ind}]$}{
      $\text{Send}_{j}(\text{ShardExecAppend}, \{T|j, \text{ind}, SSN[j]\})$ 
    }
  } \Else {
...
\par Once the transaction is replicated on the tail node, it is sent to the shard groups for execution using $\text{ShardExecAppend}$. To reduce the size of network messages, only the relevant sub-transaction is transmitted to each participating shard. Similar to before, a shard-specific sequence number $sn$ is also included. Retries are piggybacked on subsequent invocations or sent after a preset timeout, whichever happens first. \\
\begin{procedure}[H]
  \caption{ShardExecAppend($T|j$, $ind$, $sn$)}
...
\begin{procedure}[H]
  \caption{ExecAppendTransact($ind$)}
...
\begin{procedure}[H]
  ...
  $\text{Send}_{i}(\text{ReadOnlyTransact}, \{T, isRetry, crsn_T, ackBound, writeDep, lsnConst\})$ \\
  \textbf{return} $crsn_T$
\end{procedure}
The utility of $crsn_T$ and $ackBound$ are the same as in write-only transactions. For every read-only transaction, $writeDep$ captures dependency on the most recent ongoing write. This is not required in all cases. If the write key-set of all ongoing transactions are known, a dependency need only be declared on the latest transaction $X$ such that $X^{\text{write}} \cap T^{\text{read}} \neq \emptyset$. Unfortunately, this optimization is not always possible due to the way we handle read-write and dependent transactions (\S\ref{deptxn}). \\
\begin{procedure}[H]
  \caption{SessionRespRead($crsn_T$, $fence$, $data$, $numShards$)}
...
% TODO: handle ackBound and send forwards/backwards through chain for cleanup
\begin{procedure}[H]
  \caption{ReadOnlyTransact($T$, $isRetry$, $crsn_T$, $ackBound$, $writeDep$, $lsnConst$)}
  \SetKwFor{Match}{match}{with:}{end}
  \tcp{ReadOnlyTransact on manager node $i\in[1,n]$}
  \uIf{$ writeDep \geq\min(W_i^{\text{key}}|cid).num$}{
    \textbf{wait} until $writeDep \in W_i^{\text{key}}$
  }
  $fence := -1$\\
...
\section{Read-Write and Dependent Transactions} \label{deptxn}

A client-driven approach would enable the $writeDep$ optimization described in the previous section, but the tradeoff would be an increase in the number of client-manager node round trips. In view of acheiving higher throughput, we opt not to take this approach. 
...
%   \Match{$(isRetry, crsn_T.cid \in R_i, crsn_T.num \leq R_i[crsn_T.cid].num)$}{
%     $(\textbf{false}, \textbf{true}, \textbf{false}) \implies fence := \max\{fence, R_i[crsn_T.cid].lsn\}$ \\
%     $(\textbf{false}, \textbf{true}, \textbf{true}) \implies fence := R_i[crsn_T.cid].lsn$ \\
%     $(\textbf{true}, \textbf{true}, \textbf{true}) \implies fence := lsnConst$ \\
%     $(\textbf{true}, \textbf{true}, \textbf{false}) \implies fence := lsnConst$    
%   }
\end{document}
