...
\begin{procedure}[H]
  ...
  \ForEach{$op \in T|j$}{
    \tcp{versioned put exposed by storage service, these will be batched in practice}
    $\text{put}(op.key, op.value, ind)$
  }
  $shExec_j := ind$ \\
  $ssn_j := sn$ \\
...
\begin{procedure}[H]
  ...
  $data := \emptyset$\\
  \ForEach{$op \in T|j$}{
    \tcp{multi-versioned get exposed by storage service, calls batched in practice}
    $data := data \cup \{op.key \mapsto \text{get}(op.key)\}$
  }
